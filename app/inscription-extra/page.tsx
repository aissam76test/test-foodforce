...
export default function InscriptionExtra() {
  ...
  if (sent) return (
    <div style={{ textAlign: 'center', padding: '100px 40px', fontFamily: 'Poppins, sans-serif' }}>
      <div style={{ fontSize: '60px' }}>🎉</div>
      <h1 style={{ fontSize: '32px', fontWeight: 800, color: '#1a1a1a', margin: '20px 0 10px' }}>Inscription envoyée !</h1>
      <p style={{ color: '#666' }}>On vous contacte très bientôt.</p>
    </div>
  );

  return (
    <main style={{ fontFamily: 'Poppins, sans-serif', background: '#FDF0E8', minHeight: '100vh', padding: '60px 20px' }}>
      <div style={{ maxWidth: '500px', margin: '0 auto', background: 'white', borderRadius: '20px', padding: '40px', boxShadow: '0 4px 30px rgba(0,0,0,0.08)' }}>
        ...
        <h1 style={{ fontSize: '24px', fontWeight: 800, marginBottom: '4px', textAlign: 'center' }}>Créer un compte candidat</h1>
        ...
        <div style={{ display: 'flex', alignItems: 'center', marginBottom: '32px', gap: '8px' }}>
          <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: '#F47C20', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '14px' }}>1</div>
          <div style={{ flex: 1, height: '2px', background: etape === 2 ? '#F47C20' : '#eee' }} />
          <div style={{ width: '32px', height: '32px', borderRadius: '50%', background: etape === 2 ? '#F47C20' : '#eee', color: etape === 2 ? 'white' : '#999', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '14px' }}>2</div>
        </div>

        {etape === 1 && (
          <form onSubmit={handleEtape1}>
            <h2 style={{ fontSize: '16px', fontWeight: 700, marginBottom: '4px' }}>Informations de connexion</h2>
            <p style={{ fontSize: '13px', color: '#888', marginBottom: '24px' }}>Créez vos identifiants de connexion</p>
            ...
            <button type="submit" style={{ width: '100%', background: '#F47C20', color: 'white', padding: '16px', borderRadius: '10px', border: 'none', fontSize: '16px', fontWeight: 700, cursor: 'pointer', marginTop: '8px' }}>
              Continuer
            </button>
            <p style={{ textAlign: 'center', marginTop: '16px', fontSize: '13px', color: '#666' }}>
              Déjà un compte ? <a href="/auth/login" style={{ color: '#F47C20', fontWeight: 600 }}>Se connecter</a>
            </p>
          </form>
        )}
        ...
        {etape === 2 && (
          <form onSubmit={handleSubmit}>
            ...
            <p style={{ fontSize: '13px', color: '#888', marginBottom: '24px' }}>Complétez votre profil</p>
            ...
            <div style={{ marginBottom: '16px' }}>
              ...
              <select value={form.secteur} onChange={(e) => setForm({ ...form, secteur: e.target.value, metier: '' })} required
                style={{ width: '100%', padding: '12px 16px', borderRadius: '8px', border: '1.5px solid #eee', fontSize: '14px', outline: 'none' }}>
            ...
            {form.secteur && (
              <div style={{ marginBottom: '16px' }}>
                ...
                <select value={form.metier} onChange={(e) => setForm({ ...form, metier: e.target.value })} required
                  style={{ width: '100%', padding: '12px 16px', borderRadius: '8px', border: '1.5px solid #eee', fontSize: '14px', outline: 'none' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
              <button type="button" onClick={() => setEtape(1)}
                style={{ flex: 1, background: '#f5f5f5', color: '#1a1a1a', padding: '16px', borderRadius: '10px', border: 'none', fontSize: '15px', fontWeight: 600, cursor: 'pointer' }}>
                Retour
              </button>
              <button type="submit"
                style={{ flex: 2, background: '#F47C20', color: 'white', padding: '16px', borderRadius: '10px', border: 'none', fontSize: '15px', fontWeight: 700, cursor: 'pointer' }}>
                Créer mon compte
              </button>
            </div>
          </form>
