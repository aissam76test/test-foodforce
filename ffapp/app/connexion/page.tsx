...
export default function Connexion(){
 ...
 async function login(e:FormEvent){e.preventDefault();setMessage("Connexion…");const s=createClient();const {data,error}=await s.auth.signInWithPassword({email,password});if(error){setMessage(error.message);return;}const {data:p}=await s.from("profiles").select("role").eq("id",data.user.id).single();window.location.href=p?.role==="pro"?"/pro":p?.role==="extra"?"/extras":"/";}
