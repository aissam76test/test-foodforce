...
export default function Notifications(){
 ...
 return <main className="appShell"><header className="appHeader"><Link className="brand" href="/">FOOD<span>FORCE</span></Link><div className="appName">EXTRAS</div><nav><Link href="/extras">Missions</Link><Link href="/extras/mes-missions">Mes missions</Link><Link href="/extras/profil">Profil</Link></nav></header><section className="pageIntro"><span className="eyebrow">FOODFORCE EXTRAS</span><h1>Notifications.</h1><p>Les changements importants sur vos candidatures apparaissent ici.</p></section><section className="missionGrid">{items.map(n=><article className="missionCard" key={n.id}><span className={n.read_at?"tag":"tag acceptedTag"}>{n.read_at?"LUE":"NOUVELLE"}</span><h3>{n.title}</h3><p>{n.body}</p><small>{new Date(n.created_at).toLocaleString("fr-FR")}</small>{!n.read_at&&<button className="primaryBtn" onClick={()=>read(n.id)}>Marquer comme lue</button>}</article>)}</section></main>