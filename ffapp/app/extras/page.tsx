...
import { formatMissionSlot, missionHours } from "../../lib/mission-time";
...
export default function Extras(){
 ...
 return <main className="appShell">
  ...
  <section className="missionGrid">{missions.map(m=><article className="missionCard" key={m.id}><div className="missionTop"><span className="tag">DISPONIBLE</span><span>📍 {m.city}</span></div><h3>{m.tariff_grid?.job}</h3><p className="missionTime">{formatMissionSlot(m.starts_at,m.ends_at)}</p><div className="anonymous"><span>🏨</span><div><b>Établissement partenaire</b><small>Identité révélée après sélection</small></div></div><div className="missionBottom"><strong>{Number(m.candidate_rate).toFixed(2)} <small>MAD/h</small></strong><span>{m.seats} poste(s) · ≈ {(missionHours(m.starts_at,m.ends_at)*Number(m.candidate_rate)).toFixed(2)} MAD</span></div><button onClick={()=>apply(m.id)}>Postuler à cette mission</button></article>)}</section>
