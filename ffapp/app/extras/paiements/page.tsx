  ...
  useEffect(() => {
    (async () => {
      ...
      const { data, error } = await s
        ...
        .select(
          "id,mission_id,hours,candidate_hourly_rate,extra_amount,status,paid_at,created_at",
        )
        .eq("extra_id", user.id)
        .order("created_at", { ascending: false });

      if (error) {
        setMessage("Aucun paiement disponible pour le moment.");
      } else {
        setItems(data || []);
        setMessage("");
      }
  ...
  return (
    <main className="appShell">
      ...
      <section className="pageIntro">
        ...
        <h1>Mes paiements.</h1>
        <p>Retrouvez les montants générés après validation de vos heures.</p>
      </section>
      ...
      <section className="missionGrid">
        {items.map((p) => (
          <article className="missionCard" key={p.id}>
            <span
              className={p.status === "paid" ? "tag acceptedTag" : "tag"}
            >
              {p.status}
            </span>
            <h3>{Number(p.extra_amount).toFixed(2)} MAD</h3>
            <p>
              {p.hours} h × {Number(p.candidate_hourly_rate).toFixed(2)} MAD/h
            </p>
            <small>
              {p.paid_at
