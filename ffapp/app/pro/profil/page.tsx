...
export default function ProfilPro(){
 ...
 async function save(e:FormEvent){e.preventDefault();const s=createClient();const {data:{user}}=await s.auth.getUser();if(!user)return;const {error}=await s.from("profiles").update({full_name:name,phone,city}).eq("id",user.id);setMessage(error?error.message:"Profil enregistré ✅");}
