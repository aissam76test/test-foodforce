...
export default function Candidatures(){
 ...
 useEffect(()=>{load()},[]);
 async function decide(id:string,status:"accepted"|"rejected"){const s=createClient();const {error}=await s.rpc("decide_application",{p_application_id:id,p_status:status});if(error)setMessage("Erreur : "+error.message);else{setMessage(status==="accepted"?"Extra accepté ✅":"Candidature refusée");load();}}
