...
export default function Inscription(){
 ...
 async function signup(e:React.FormEvent){e.preventDefault();setMessage("Création du compte…");const s=createClient();const {data,error}=await s.auth.signUp({email,password});if(error){setMessage(error.message);return;}if(data.user){const {error:pError}=await s.from("profiles").insert({id:data.user.id,role,full_name:name,city});if(pError){setMessage(pError.message);return;}}setMessage("Compte créé. Vérifiez votre email si une confirmation est demandée.");}
